...
from .predict import _predict, edit_src_by_tags
...
def predict_verbose(
    ...
    for itr in range(n_iteration):
        print(f"Iteration {itr}. the number of to_be_processed: {len(to_be_processed)}")
        ...
        for i, orig_id in enumerate(current_orig_idx):
            ...
            iteration_log[orig_id].append({"src": edited_srcs[i], "tag": None})
