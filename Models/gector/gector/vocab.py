...
def build_vocab(
    train_dataset: GECToRDataset, n_max_labels: int = 5000, n_max_d_labels: int = 2
):
    ...
    def get_high_freq(freq: dict, n_max: int):
        descending_freq = sorted(freq.items(), key=lambda x: x[1], reverse=True)
        high_freq = [x[0] for x in descending_freq][:n_max]
        if len(high_freq) < n_max:
            print(
                f"Warning: the size of the vocablary: {len(high_freq)} is less than n_max: {n_max}."
            )
        return high_freq
