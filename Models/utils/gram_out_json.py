...
def load_token_list(token_path: str):
...
def get_cleaned_token_list(token_path: str = "../gector/data/token_labels.txt"):
    ...
    fundamental_pattern = (
        r"[$](KEEP|TRANSFORM_CASE|TRANSFORM_SPLIT_HYPHEN|MERGE_HYPHEN|MERGE_SPACE)"
    )
...
def get_scrs_tok(inner_dict: Dict, tok_list: List):
...
def get_tag_grammar(tag_list: List):
    ...
    for p in preps_tag_list:
        tag_grammar_dict[p] = {"category": "전치사", "desc": "PREPOSITION"}

    return tag_grammar_dict


def get_phase_1_data(score: float, check_data: Dict):
...
def get_phase_2_data(p1_data: Dict, check_data: Dict, ctl: List, tag_grammar: Dict):
    tag_gram_dict = {"tag_grammar_info": []}
...
def create_json(phase: str, out_path: str, score: float, check_data: Dict):
